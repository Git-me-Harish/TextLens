            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                onClick={async () => {
                  toast.dismiss(t.id);
                  try {
                    // A completed job of the same type can be shown directly.
                    // Otherwise, run this tool against the stored source file.
                    if (detail.existing_job_type === jobType && detail.existing_job_status === "completed") {
                      const { data: existing } = await api.get(`/jobs/${detail.existing_job_id}`);
                      setJob(existing);
                    } else {
                      const reuse = new FormData();
                      reuse.append("job_type", jobType);
                      const { data: nextJob } = await api.post(`/jobs/${detail.existing_job_id}/reuse`, reuse);
                      setJob(nextJob);
                      toast("Using existing source, processing...", { icon: "..." });
                    }
                  } catch { toast.error("Failed to use the existing source"); }
                }}

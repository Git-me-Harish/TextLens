...
async def get_my_stats(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    completed = (await db.execute(select(func.count(OCRJob.id)).where(OCRJob.user_id == user.id, OCRJob.status == "completed"))).scalar()
    agent_runs = (await db.execute(select(func.count(AgentRun.id)).where(AgentRun.user_id == user.id))).scalar()

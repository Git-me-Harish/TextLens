...
const STYLES = [
  {
    ...
    label:    "Executive",
    icon:     "📋",
    desc:     "3–5 sentence overview for quick decisions",
    ratio:    0.15,
    format:   "paragraph",
  },
  {
    ...
    label:    "Bullet Points",
    icon:     "•",
    desc:     "Scannable list of key takeaways",
    ratio:    0.25,
    format:   "bullets",
  },
  {
    ...
    label:    "Key Topics",
    icon:     "🏷",
    desc:     "Main themes with brief explanations",
    ratio:    0.20,
    format:   "topics",
  },
  {
    ...
    label:    "Detailed",
    icon:     "📄",
    desc:     "Comprehensive summary covering all sections",
    ratio:    0.50,
    format:   "paragraph",
  },
];
  ...
  return paragraphs.map((para, i) => {
    ...
    return (
      <div key={i} style={{ marginBottom: "1rem", padding: "0.875rem 1rem", background: "var(--paper)", border: "1px solid var(--border)", borderLeft: "3px solid var(--accent)", borderRadius: "0 8px 8px 0" }}>
        ...
        {rest && (
          <div style={{ fontSize: "0.82rem", color: "var(--ink-secondary)", lineHeight: 1.6 }}>
            {rest}
          </div>
        )}
      </div>
...
function formatAsParagraph(text) {
  ...
  return paragraphs.map((p, i) => (
    <p key={i} style={{ margin: "0 0 0.625rem", fontSize: "0.875rem", lineHeight: 1.7, color: "var(--ink-secondary)" }}>
      {p}
    </p>
  ));
}
...
function SummaryResult({ text, styleId }) {
  ...
  let rendered;
  if (style.format === "bullets") rendered = formatAsBullets(text);
  else if (style.format === "topics") rendered = formatAsTopics(text);
  else rendered = formatAsParagraph(text);
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">AI-powered document summarization — choose your format</p>
        </div>
        {job && (
      ...
      {!job && (
        <>
          ...
          <div style={{ marginBottom: "1.5rem" }}>
            <div style={{ fontSize: "0.78rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "var(--ink-muted)", marginBottom: "0.625rem" }}>
              Summary style
            </div>
              ...
              {STYLES.map(s => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    background: selectedStyle === s.id ? "var(--paper-2, #f0f4ff)" : "var(--paper)",
                    cursor: "pointer",
                ...
                >
                  ...
                  <div style={{ fontWeight: 700, fontSize: "0.82rem", color: selectedStyle === s.id ? "var(--accent)" : "var(--ink)", marginBottom: 3 }}>
                    {s.label}
                  </div>
                  <div style={{ fontSize: "0.7rem", color: "var(--ink-muted)", lineHeight: 1.4 }}>
                    {s.desc}
                  </div>
                </div>
              ))}
          ...
          <div {...getRootProps()} className={`dropzone ${isDragActive ? "active" : ""}`}>
            ...
            {uploading ? (
              ...
              <>
                <div className="dropzone-icon"><Upload size={26} /></div>
                <div className="dropzone-title">Drop a PDF to summarize</div>
                <div className="dropzone-sub">
                  Will generate a {currentStyle?.label?.toLowerCase()} summary — max 50 MB
                </div>
              </>
            )}
          </div>
      ...
      {job && summaryText && (
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <div className="card" style={{ padding: "0.875rem 1.25rem", display: "flex", alignItems: "center", gap: "1rem", flexWrap: "wrap" }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600, fontSize: "0.88rem", color: "var(--ink)", marginBottom: 2 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={download} className="btn btn-outline btn-sm">
